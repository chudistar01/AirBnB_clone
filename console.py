...
class HBNBCommand(cmd.Cmd):
    ...
    def do_destroy(self, arg):
        """
        Delete class instance a given id
        """
        argl = parse(arg)
        objdict = storage.all()  
        if len(argl) == 0:
    ...
    def do_all(self, arg):
        """
        string representation of all instances of a
        given class
        """
        argl = parse(arg)
    ...
    def do_count(self, arg):
        """
        retrieves number of instances in a class
        """
        argl = parse(arg)
        count = 0
        for obj in storage.all().values():
        ...
        if len(argl) == 4:
            ...
            for k, v in eval(argl[2]).items():
                if (k in obj.__class__.__dict__.keys() and
                    ...
                    valtype = type(obj.__class__.dict__[k])
